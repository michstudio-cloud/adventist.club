...
**Asistente de escritorio, flujo (2026-09-25, `ab80f3f`):** los certificados empiezan en el catálogo («Crear certificado» en la vista previa de cada parche → `/certificates/new?honor=<slug>`, que abre en el paso 2); el paso 1 en escritorio invita a ir al catálogo; cada paso termina en un botón grande «Siguiente: …», el 3 en «Generar» y el 4 en «Descargar»; el paso siguiente también se abre desde la lista; la barra flotante con flechas no se monta en escritorio; toast abajo a la derecha. Móvil intacto.

**Traducciones (2026-09-25):** backend `5174fd1` fusionado y cargado en Neon (186 listas, 1 755 requisitos, 364 nombres; idempotente: segunda corrida sin cambios). Conquistadores: español e inglés completos en las 796 especialidades con requisitos (13 no tienen lista en ningún idioma: no hay nada que traducir); portugués (pt-BR) en las primeras 120 por slug. Verificado en el API: `aves-de-rapina?locale=es` → es; `?locale=pt` → pt-BR («Raptores»). Convención `source='traduccion-no-oficial-<origen>'` (CC BY-SA 3.0 y URL de la wiki; la página oficial 100 % traducida la reemplaza en el sitio conservando ids y progreso). Herramientas en `migrations/catalog_tools/` (`requirement_translation_gaps.py`, `export_translation_sources.py`, `validate_requirement_translations.py`), importador `import_requirement_translations.py`, procedimiento en `docs/TRADUCCIONES.md`, pendientes en `data/requirement_translations/pending.json`. **Fase B terminada y cargada en Neon (2026-09-25):** pt-BR de las 796 especialidades de Conquistadores con lista (6 016 requisitos, 303 nombres) y de los 164 awards de Aventureros (989 requisitos, 164 nombres, 164 descripciones). **13 sin lista, resueltas (backend `b6c31cc`, cargado en Neon):** 12 tienen ya lista es/en/pt-BR (oficial: wiki para Comida cruda = «Bushcraft Food», ADRA bronce/plata/oro = «ADRA Annual Appeal Collector», Scrapbooking (FL) avanzado; PDF de la SPD para Procesar texto; guiasmayores.com para Nudos avanzado, Ensamblaje de computadoras y Primeros auxilios de San Juan; Wikibooks CC BY-SA 4.0 para Desarrollo rural/urbano (ADRA 2005, descontinuados); MDAWiki (manual DSA) para La unidad en el cuerpo de Cristo; lo demás como traducción no oficial). **Noken sigue sin requisitos: no están publicados en ninguna fuente.** Nuevo `import_official_requirements.py` (`data/official_requirements/`), `wiki_title` añadidos, `import_wiki_requirements.py` ya no duplica una lista cuando ese idioma tiene otra oficial, créditos en la hoja PDF. Categorías: 11 nombres pt-BR cargados (`import_category_translations.py`, `data/category_translations.json`) y corregido un error de producción (las 6 categorías de Aventureros estaban como `pt` en vez de `pt-BR`; por eso con `?locale=pt` las de Conquistadores salían en español). Inventario final en Neon: Conquistadores 808/809 con lista en es, en y pt-BR (falta solo Noken); Aventureros 164/164 en los tres idiomas; categorías completas. Decisiones del propietario: fuentes de terceros (Wikibooks, MDAWiki) aceptadas como origen; ADRA muestra la misma lista en los tres niveles; «Comida cruda» conserva su nombre (la wiki la llama «Comida para supervivencia»); Doctrinales en pt-BR = «Doutrinárias» (la DSA usa «Ensinos Bíblicos»); revisar los 95 nombres de nudos en en/pt-BR; en `album-de-recortes-florida-conference` la lista es de guiasmayores perdió los sub-incisos del requisito 4. Fuera del importador: nombre pt-BR de 5 categorías (ADRA, Doctrinales, Servicios Comunitarios, Asociación de Florida, Maestrías).
Decisiones del propietario: etiqueta «traducción no oficial» en la interfaz (hoy solo sale «Sólo disponibles en Inglés» cuando no hay lista); decimales en español con punto (16.1 km) o coma; nombres pt-BR sin equivalente oficial DSA (Raptores/Aves de rapina, Animais peçonhentos, Mergulho autônomo, Busca e resgate, Combate à fome, Arte urbana, Áudio e som/Sonoplastia, Olaria); nombres en inglés (Scrapbooking de Florida coincide con el de la GC: ¿añadir «(Florida Conference)»?; Raw Food, Textile Pattern Making, St John First Aid, ADRA Fundraising); «La Voz de la Profecía» o «La Voz de la Esperanza»; «Alive Bible» = «Biblia viva»; revisar vocabulario de gimnasia y clavados; defectos del origen que se dejaron tal cual (pista y campo 2 y 4 sin tabla de tiempos; liderazgo al aire libre avanzado 12 sin lista; «100 square feet (30.5 m²)» en Jardinería; listas de guiasmayores.com mal extraídas, sobre todo «Sirviendo a las comunidades»).

**Barra superior (2026-09-25, frontend `a76d7cc`, 54 tests):** ministerio como botón circular con solo el emblema junto al botón de tema (menú de emblemas con `aria-label`, clubes por nombre si hay varios, enlace a la ficha del club o «Busca un club»; sin sesión, «Inicia sesión para cambiar de ministerio»), idioma como bandera con menú (SVG propios en `public/flags/`: España, EE. UU., Brasil; el propietario puede preferir México cambiando `es.svg`), detección por `Accept-Language` en la primera visita sin cookie (`localeToRemember`, `proxy.ts` fija la cookie; elegir en el menú manda). La tarjeta de ministerio y la fila ES · EN · PT dejan la barra lateral; el emblema del riel colapsado también (duplicaba el de la barra superior). `DeskContext` sigue en el kit sin uso. Límites: las páginas estáticas envían el HTML en español y el armazón cambia al cargar; la portada no está traducida; si la persona cambia luego el idioma del navegador no se vuelve a detectar (manda la cookie).

**Fase 5 publicada (2026-09-25, frontend `3ac4c55`, 70 tests):** e2e 02/06/11 con sesión y jornada 13 «escritorio» (13/13 en verde), `scripts/e2e/desk-audit.mjs` (152 capturas: 38 estados × 1440/1024 × claro/oscuro) y `scripts/e2e/contrast.mjs` (contraste sobre píxeles reales: 625 → 10 medidas bajo AA; tabla en `docs/ACCESIBILIDAD_ESCRITORIO.md`); tokens `--muted-foreground`, `--segment-foreground`, `--link` más oscuros solo desde 1024 px, nuevos `--control-line` y `--brand-shade`; `<html lang>` sigue al idioma de la interfaz; teclado: ⌘K cierra la paleta, `cq-datatable` con una sola parada por fila, `cq-segmented` con flechas, foco visible en campos; regresión: parche que pisaba su nombre, ayudas cortadas en `cq-steps`, KPI 2+1 a 1024, marcador cortado en el riel de admin; alias `.cq-roll-foot`, `.cq-pp-avatar`, `--elev-*` retirados, 113 rellenos a `--space-*`, 6 clases muertas. Fusionado con el módulo de eventos de la otra sesión (`522c70b`, `events.adventist.club`). **Con esto el plan de escritorio (fases 0–5) está completo.**
Pendiente del kit: 141 tamaños sueltos en TSX (`text-[Npx]`, `min-h-[Npx]`; lista por archivo en el informe de la fase 5: tokens-specimens 8, org-detail 7, requirements-inplace 7, class-matrix 6…); contraste del teléfono (tokens AA solo aplican desde 1024 px por la regla «móvil intacto»: extenderlo cambia el teléfono, decisión del propietario); 10 medidas bajo AA en pantallas del teléfono vistas en el armazón (contador y píldora de la portada del álbum, «sem.» en `/panel/club`); tres detalles por pantalla: tres bloques «Mi club» en el panel del director (uno de escritorio y dos del teléfono), píldora de ministerio de la tarjeta del club a todo el ancho, columnas Director/correo cortadas con «…» en `/admin/clubes` a 1440.
