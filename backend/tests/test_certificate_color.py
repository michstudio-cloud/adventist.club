"""«Especialidad · Marco multicolor» (replica-especialidad-color): the colour sibling of the gold
design, compiled by the same tool; the owner (2026-09-24) wants it first in the picker and as the
default honor certificate."""
import re
import sys
...
SLUG = "especialidad-color"
LOCALES = ("es", "en", "pt", "fr")
PACKAGE = Path.home() / "Documents" / "DEEL" / "certificados-diseno" / "replica-especialidad-color"
...
def test_installed_first_class_with_four_languages_signatures_and_a_flat_background():
    template = load_template(SLUG)
    assert (template.width_pt, template.height_pt) == (792.0, 612.0)
    assert template.meta["title"] == "Especialidad · Marco multicolor"
    assert template.meta["kinds"] == ["honor"] and template.meta["source"] == "replica-especialidad-color"
...
def test_it_is_the_default_honor_certificate():
    assert DEFAULT_CERTIFICATE_TEMPLATE == SLUG
