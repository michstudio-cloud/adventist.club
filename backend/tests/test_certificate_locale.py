...
async def test_issuing_stores_the_locale_and_exposes_it(client, factory, world, issuer):
    ...
    issued = await client.post(f"{ENROLLMENTS}/{eid}/certificate", headers=world["director"]["headers"], json={
        "template": "especialidad-reticula-verde", "locale": "fr", "issued_date": "2026-09-22"})
    assert issued.status_code == 201, issued.text
    body = issued.json()
    assert body["locale"] == "fr" and body["template_slug"] == "especialidad-reticula-verde"
    ...
    assert verified["locale"] == "fr" and verified["template_slug"] == "especialidad-reticula-verde"
    ...
    assert listed["locale"] == "fr" and listed["template_slug"] == "especialidad-reticula-verde"
...
async def test_a_locale_the_template_does_not_speak_is_refused(client, factory, world, issuer):
    ...
    for template, locale in (("especialidad-basica", "fr"), ("especialidad-reticula-verde", "de")):
...
async def test_render_prints_in_the_stored_locale_unless_asked_otherwise(client, factory, world, issuer, monkeypatch):
    ...
    french = load_template("especialidad-reticula-verde").strings["fr"]["church_name"]

    stored = await client.post(RENDER, json={"template": "especialidad-reticula-verde", "format": "svg",
                                             "certificate_no": folio})
    assert stored.status_code == 200, stored.text
    assert "22 septembre 2026" in _texts(stored.text) and french in _texts(stored.text)

    english = await client.post(RENDER, json={"template": "especialidad-reticula-verde", "format": "svg",
...
async def test_the_assistant_batch_keeps_the_template_it_rendered(client, factory, world, issuer):
    ...
    chosen = await _prototype(client, factory, template="especialidad-reticula-verde")
    assert chosen.status_code == 201, chosen.text
    folio = chosen.json()[0]["certificate_no"]
    verified = (await client.get(f"/api/v1/certificates/verify/{folio}")).json()
    assert verified["valid"] is True and verified["template_slug"] == "especialidad-reticula-verde"
    ...
    assert stored["name"] == "especialidad-reticula-verde" and stored["supports_svg"] is True
    ...
    english = await _prototype(client, factory, template="especialidad-reticula-verde", locale="en")
    ...
    assert verified["locale"] == "en" and verified["template_slug"] == "especialidad-reticula-verde"
    unsupported = await _prototype(client, factory, template="especialidad-reticula-verde", locale="de")
